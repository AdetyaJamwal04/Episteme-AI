# VeriFact — Research Orchestrator (Control Plane Specification)

## 1. Purpose & System Role

The **Research Orchestrator** is the central control plane of VeriFact. It converts the verification process from a rigid, one-shot retrieval pipeline into an **adaptive, hypothesis-driven sequential decision process**.

```text
Static Pipeline (Insufficient):
Claim → Search Queries → Retrieve Top-K → NLI Stance → Verdict

Adaptive Agent (VeriFact Orchestrator):
