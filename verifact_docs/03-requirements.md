# VeriFact — Product & System Requirements

## 1. Purpose

This document translates the VeriFact product vision and problem definition into explicit, testable requirements.
...
The system SHALL detect input language and enforce English-first processing in MVP. Non-English claims ($\ge 0.85$ confidence) SHALL be rejected with structured code `UNSUPPORTED_LANGUAGE` as specified in [00-language-and-scope.md](file:///c:/Projects/VeriFact/verifact_docs/00-language-and-scope.md).
...
Each candidate evidence item SHALL be classified relative to an atomic claim according to the canonical taxonomy in [00-canonical-enums.md](file:///c:/Projects/VeriFact/verifact_docs/00-canonical-enums.md):
...
The system SHALL derive an overall verdict from the state of the atomic claims and their evidence according to [00-canonical-enums.md](file:///c:/Projects/VeriFact/verifact_docs/00-canonical-enums.md).
...
This requirements document is the contract from which the VeriFact domain model and system architecture should be derived.
