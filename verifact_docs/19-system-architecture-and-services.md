# VeriFact — System Architecture and Services Specification

## 1. Purpose & Architectural Overview

This document specifies the technical system architecture for VeriFact. It defines:
...
All backend application code resides within the `verifact` namespace package. Cyclic dependencies between modules are strictly forbidden and enforced via CI linting rules.
...
VeriFact strictly separates **metadata & relational entities** from **raw document text** and **vector embeddings**:
...
```text
┌─────────────────────────┐     gRPC      ┌─────────────────────────┐
│   VeriFact API Gateway  │ ────────────> │ Claim & Research Agent  │
