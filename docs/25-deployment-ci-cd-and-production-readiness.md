# Episteme — Deployment, CI/CD, and Production Readiness Specification

## 1. Purpose & Production Engineering Objectives

This document specifies the deployment topology, container packaging, automated CI/CD pipeline, database migration workflows, observability stack, and disaster recovery procedures required to run Episteme in production.
...
Episteme employs multi-stage Docker builds to produce minimal, hardened production container images based on `python:3.12-slim`:
...
RUN useradd -u 10001 -m episteme_user

COPY --from=builder /install /usr/local/lib/python3.12/site-packages
COPY --chown=episteme_user:episteme_user ./episteme /app/episteme
COPY --chown=episteme_user:episteme_user ./alembic /app/alembic
COPY --chown=episteme_user:episteme_user alembic.ini /app/alembic.ini
...
ENTRYPOINT ["uvicorn", "episteme.main:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "4"]
```
...
| **2. Type Check** | `mypy episteme` | Strict mode exit 0 | 2 mins |
| **3. Security Scan** | `bandit -r episteme -ll && trivy fs .` | Zero high/critical CVEs | 2 mins |
| **4. Unit Tests** | `pytest tests/unit --cov=episteme --cov-fail-under=85` | 100% pass, ≥85% code coverage | 3 mins |
| **5. Benchmark Gate**| `python -m tests.benchmarks.run_benchmark` | Macro F1 ≥ 0.88, ECE ≤ 0.08 | 5 mins |
| **6. Image Build** | `docker build -t episteme:sha-$(git rev-parse --short HEAD) .` | Successful build | 4 mins |
...
Episteme exposes Prometheus metrics on `/metrics`:

```text
Metric Name                               | Type      | Description
──────────────────────────────────────────┼───────────┼─────────────────────────────────────────────
episteme_requests_total                   | Counter   | Total HTTP verification requests by mode & status
episteme_request_duration_seconds         | Histogram | Request latency distribution by mode
episteme_verdict_distribution_total       | Counter   | Verdict counts (SUPPORTED, REFUTED, etc.)
episteme_search_provider_latency_seconds  | Histogram | Search API call latency by provider (Tavily/Brave)
episteme_search_provider_errors_total     | Counter   | Search provider HTTP 429/5xx errors
episteme_nli_inference_duration_seconds   | Histogram | Local DeBERTa batch inference duration
episteme_estimated_request_cost_usd       | Summary   | Rolling estimated monetary cost per request
```

### Critical Alerting Thresholds:
- **P0 Alert (PagerDuty)**: `episteme_search_provider_errors_total` rate exceeds 5% over 5 minutes (Search Outage).
...
Before opening Episteme to public user traffic, verify:
